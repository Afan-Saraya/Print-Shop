...
// tabs
const tabs = ["Sve Kategorije", "Majice", "Šolje", "Kesice"];
...
const ProductArea = () => {
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (activeTab === 'Sve Kategorije') {
      product_items = products.data
    }
    else if (activeTab === 'Majice') {
      product_items = products.data.filter(p => p.category.name === 'Majice')
    } else if (activeTab === 'Šolje') {
      product_items = products.data.filter(p => p.category.name === 'Šolje')
    } else if (activeTab === 'Kesice') {
      product_items = products.data.filter(p => p.category.name === 'Kesice')
    } else {
      product_items = products.data;
    }
    content = <>
      <div className="row align-items-end">
        <div className="col-xl-6 col-lg-6">
          <div className="tp-section-title-wrapper-4 mb-40 text-center text-lg-start">
            <span className="tp-section-title-pre-4">Proizvodi</span>
            <h3 className="tp-section-title-4">Odaberi Proizvod i Počni Dizajniranje</h3>
          </div>
        </div>
        <div className="col-xl-6 col-lg-6">
